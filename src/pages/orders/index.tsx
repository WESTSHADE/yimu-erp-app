...
const StatusMap: Record<ORDERS.OrderStatus, ReactNode> = {
  pending: (
    ...
    >
      <IconInfoCircle
        style={{ marginRight: 6, verticalAlign: "baseline", fontSize: 12 }}
      />
  ...
  processing: (
    ...
    >
      <IconClockCircle
        style={{ marginRight: 6, verticalAlign: "middle", fontSize: 12 }}
      />
...
const Orders = () => {
  ...
  const getOrdersList = async (
    ...
    await getOrders(startTime, endDate, 1, 20, filter).then((res) => {
      ...
      if (search) {
        new_ordersList = [...new_ordersList].filter((item) =>
    ...
    <div>
      ...
      >
        ...
        >
            ...
            >
              {filterValue?.startTime && filterValue?.endTime
                ? `Select: ${pacificTime(filterValue?.startTime).format(
                    "MM-DD-YYYY"
                  )},${pacificTime(filterValue?.endTime).format("MM-DD-YYYY")}`
                : `${
          ...
          >
            {[{ label: "All", value: "all" }, ...OrderStatusList].map(
              (item) => {
                return (
                  <PCButton
                    type="secondary"
                    style={
                      searchOption.status == item.value
                        ? {
                            marginRight: 8,
              ...
              const orderData: DataType = [
                {
                  ...
                  value: item.paymentTime
                    ? formatToLocalTime(item.paymentTime, "MM-DD-YYYY HH:mm:ss")
                    : "",
                },
                {
                  label: "Status",
                  value: StatusMap[item.status as ORDERS.OrderStatus] ? (
                    StatusMap[item.status as ORDERS.OrderStatus]
                  ) : (
                    <></>
                  ),
                },
